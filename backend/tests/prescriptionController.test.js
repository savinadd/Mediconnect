...
describe("Prescription Controller", () => {
  ...
  describe("getPrescriptionsByDoctor", () => {
    ...
    it("returns rows on success", async () => {
      const sample = [{ id:1, drug_name:"X", patient_name:"P", dosage:"d", instructions:"i", prescribed_at:"t", end_date:null }];
      db.query
  ...
  describe("getPrescriptionsForPatient", () => {
    it("splits active vs history and returns them", async () => {
      ...
      const rows = [
        { id: 1, name: "A", doctor: "D", dosage:"", instructions:"", prescribed_at: now.toISOString(), end_date: past },
        { id: 2, name: "B", doctor: "D2", dosage:"", instructions:"", prescribed_at: now.toISOString(), end_date: future },
      ];
    ...
    it("throws on DB error", async () => {
      db.query.mockRejectedValue(new Error());
      await expect(getPrescriptionsForPatient(req, res)).rejects.toBeInstanceOf(InternalServerError);
    });
  });
});
