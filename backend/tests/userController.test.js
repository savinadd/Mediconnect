...
describe("userController", () => {
  ...
  describe("getUserProfile", () => {
    ...
    it("returns full patient profile", async () => {
      ...
      db.query
        .mockResolvedValueOnce({ rows: [{ id: 2, email: "x" }] })
        .mockResolvedValueOnce({ rows: [{ first_name: "A", last_name: "B", birth_date: "d", phone: "p", address: "a", blood_type: "O", height: "h", weight: "w", allergies: "n", government_id: "G" }] });
      await getUserProfile(req, res);
      expect(res.json).toHaveBeenCalledWith(
