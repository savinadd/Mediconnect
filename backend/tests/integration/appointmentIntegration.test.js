...
describe("Appointment scheduling flow", () => {
  ...
  afterAll(async () => {
    // clean up: appointments, availability, users, etc.
    await db.query("DELETE FROM appointments WHERE patient_id IN (SELECT id FROM patients WHERE user_id=(SELECT id FROM users WHERE email='pat@example.com'))");
    await db.query("DELETE FROM doctor_availability WHERE doctor_id IN (SELECT id FROM doctors WHERE user_id=(SELECT id FROM users WHERE email='doc@example.com'))");
    await db.query("DELETE FROM patients WHERE user_id=(SELECT id FROM users WHERE email='pat@example.com')");
    await db.query("DELETE FROM doctors  WHERE user_id=(SELECT id FROM users WHERE email='doc@example.com')");
