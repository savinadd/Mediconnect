...
describe('symptomController', () => {
  ...
  describe('addPatientSymptom', () => {
    it('throws BadRequestError if validation fails', async () => {
      await expect(addPatientSymptom(req, res)).rejects.toBeInstanceOf(BadRequestError);
    });
    ...
    it('throws NotFoundError if patient not found', async () => {
      ...
      await expect(addPatientSymptom(req, res)).rejects.toBeInstanceOf(NotFoundError);
    });
    ...
    it('creates new symptom and logs activity', async () => {
      ...
      expect(db.query).toHaveBeenCalledWith(expect.stringContaining('INSERT INTO symptoms'), [
        'Fever',
        'High fever',
      ]);
    ...
    it('wraps DB error in InternalServerError', async () => {
      ...
      await expect(addPatientSymptom(req, res)).rejects.toBeInstanceOf(InternalServerError);
    });
  });
  ...
  describe('getPatientSymptomHistory', () => {
    it('throws NotFoundError if no patient record', async () => {
      ...
      await expect(getPatientSymptomHistory(req, res)).rejects.toBeInstanceOf(NotFoundError);
    });
    ...
    it('wraps errors in InternalServerError', async () => {
      ...
      await expect(getPatientSymptomHistory(req, res)).rejects.toBeInstanceOf(InternalServerError);
    });
  });
});
