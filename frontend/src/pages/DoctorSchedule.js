      ...
      (t) => (
        <div className="confirm-toast">
          ...
          <div className="confirm-buttons">
            <button onClick={() => { resolve(true); toast.dismiss(id); }}>Yes</button>
            <button onClick={() => { resolve(false); toast.dismiss(id); }}>No</button>
          </div>
        </div>
      ),
      { duration: Infinity }
  ...
  const fetchEvents = async () => {
    try {
      ...
      const bookedTimes = new Set(
        reqRaw.map(r =>
          `${new Date(r.start_time).getTime()}-${new Date(r.end_time).getTime()}`
        )
      );
      const filteredAvail = availRaw.filter(a =>
        !bookedTimes.has(
          `${new Date(a.start_time).getTime()}-${new Date(a.end_time).getTime()}`
        )
      );
    ...
    if (event.type === "availability") {
      ...
      if (event.status === "pending") {
        ...
        try {
          ...
          if (!res.ok) throw new Error(`${action.charAt(0).toUpperCase() + action.slice(1)} failed`);
