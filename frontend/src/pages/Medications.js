...
const Prescriptions = () => {
  ...
  useEffect(() => {
    ...
    else if (role === "doctor") fetchDoctorPrescriptions();
  }, [isLoggedIn, role]);
  ...
  useEffect(() => {
    if (message) {
      ...
      const cleanup = setTimeout(() => setMessage(""), 2200);
      return () => {
  ...
  return (
    <>
      {message && (
        <div
          className={`popup-alert ${message.toLowerCase().includes("success") ? "success" : "error"} ${fadeOut ? "fade-out" : ""}`}
        >
          {message}
        </div>
      )}
            ...
            activePrescriptions.map((prescription) => (
              <div className="prescription-card" key={prescription.id}>
                ...
                <div className="prescription-details">
                  ...
                  {role === "doctor" && (
                    <button className="end-button" onClick={() => handleEndPrescription(prescription.id)}>
                      End Prescription
                    </button>
                  )}
                </div>
